---
title: "provider network와 tenant network 개념 정리"
description: "provider network와 tenant network 개념 정리"
---

# provider network와 tenant network 개념 정리

작성시간 - origin: 2026년 2월 25일 오후 7:12
작성시간 - 속성수정: 2026년 2월 25일 오후 7:12 (GMT+9)
...
[물리 네트워크(스위치/라우터)]
          ...
          | (직접 매핑/연결)
          |
[Provider Network]  <--- 운영자 영역 (외부/물리와 연결)
          |
          v (라우팅/NAT 게이트)
[Virtual Router]
          |
          v
[Tenant Network]    <--- 테넌트 영역 (VM 내부 통신)
          |
          v
[VM Ports / VMs]
```
...
### (1) 테넌트 내부 통신(East-West)

같은 tenant network에 붙은 VM끼리는 그냥 L2/L3로 통신합니다.

```
VM1(10.0.0.5)  ---- Tenant Network ----  VM2(10.0.0.6)
```
...
```
VM1 -- TenantNet A --\
                      >-- Virtual Router -- TenantNet B -- VM2
VM2 -- TenantNet B --/
```
...
```
VM(사설IP) -> Tenant Network -> Virtual Router(SNAT) -> Provider/External -> Outside
```
...
```
Outside -> Provider/External
        -> Virtual Router(DNAT or Floating IP)
        -> Tenant Network -> VM
```

---

## Provider vs Tenant 차이 정리
...
[https://docs.openstack.org/neutron/2024.2/admin/ovn/refarch/provider-networks.html](https://docs.openstack.org/neutron/2024.2/admin/ovn/refarch/provider-networks.html)
