---
title: "OVS와 VXLAN으로 VPN 같은 가상 네트워크 만들기"
description: "OVS와 VXLAN으로 VPN 같은 가상 네트워크 만들기"
---

# OVS와 VXLAN으로 VPN 같은 가상 네트워크 만들기

작성시간 - origin: 2026년 1월 16일 오후 5:10
작성시간 - 속성수정: 2026년 1월 16일 오후 5:10 (GMT+9)

# 요구사항

- VM 세개를 만들어서 각각 가상 NIC를 100,200,300으로 만들어서 OVS를 설치하고 host에서 ping으로
    - 가상 네트워크에다가 ping이 가도록.
    - 가상 NIC끼리 서로 통신이 되도록 만드는 것.

⇒ OVS로 VPN서비스를 만든다. (vxlan으로)

**vm3개를 만들고 ovs와 vxlan으로 vpn 같은 가상 네트워크를 만들어보기**
...
![image.png](images/ovs_vxlan_vpn/image.png)
...
- VXLAN의 **VNI(Virtual Network Identifier)** 값 입니다
...
![image.png](images/ovs_vxlan_vpn/image 1.png)

# 트러블 슈팅

위의 구조에서 Node1 → Node2로 설정한 IP인 10.0.0.200/24로 핑을 날렸을 때 문제가 발생했습니다.

방화벽이나 네트워크 설정이나 의도한 대로 이미 잘 되어 있는 것을 확인 했고, 그럼에도 되지 않아 네트워크를 구성할 때부터 문제가 생겼음을 알 수 있었습니다

다음 명령어를 통해 어떤 문제가 발생했는 지를 살펴봅니다

```java
sudo tcpdump -i ens18 udp port 4789
```

- vxlan에서는 4789 포트에서 터널링을 위한 처리가 이뤄지고 udp로 감싸진채로 처리됩니다
...
![image.png](images/ovs_vxlan_vpn/image 2.png)
...
![image.png](images/ovs_vxlan_vpn/image 3.png)
...
- STP는 이 루프를 감지해서 특정 경로를 논리적으로 차단(Blocking)했다가, 주 경로가 끊기면 다시 여는 역할을 합니다.
    - STP가 동작하면 위와 같이 물리적으로 루프 구조인 네트워크에서 특정 포트를 차단상태로 바꾸어 논리적으로 루프가 발생하지 않게 됩니다

설정 후 들어오는 트래픽에 대해서도 확인해보면 vni 10으로 해서 잘 오고갑니다.

```java
sudo tcpdump -i ens18 udp port 4789
```

![image.png](images/ovs_vxlan_vpn/image 4.png)

![image.png](images/ovs_vxlan_vpn/image 5.png)

![image.png](images/ovs_vxlan_vpn/image 6.png)
...
![image.png](images/ovs_vxlan_vpn/image 7.png)

![image.png](images/ovs_vxlan_vpn/image 8.png)
...
![image.png](images/ovs_vxlan_vpn/image 9.png)
...
**즉 두가지 다 하나의 큰 소프트웨어를 둬서,
특정 VTEP이 생성이 됐을 때 이를 감지하고 이 VEP에 대한 네트워크 정보를 나눠주는 주체를 둬서 처리한다는 게 동일 한 것 같습니다**

- **"중앙에서 정보를 관리하고 배포하는 주체"**를 둬서, 전통적인 이더넷의 **"소문내서 찾기(Flood & Learn)"** 방식을 **"미리 알고 알려주기(Push & Direct)"** 방식으로 변경

# 참조

https://kujung.tistory.com/entry/STP%EC%8A%A4%ED%8C%A8%EB%8B%9D-%ED%8A%B8%EB%A6%AC-%ED%94%84%EB%A1%9C%ED%86%A0%EC%BD%9C%EC%9D%98-%EA%B8%B0%EB%B3%B8-%EA%B0%9C%EB%85%90%EA%B3%BC-%EC%82%AC%EC%9A%A9-%EC%9D%B4%EC%9C%A0#google_vignette

https://blog.innern.net/40